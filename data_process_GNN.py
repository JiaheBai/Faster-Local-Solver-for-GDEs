...
def main():
    ...
    parser.add_argument('--dataset', default='ogbn-arxiv', help='dateset.')
    parser.add_argument('--delnum', type=int, default=950000, help='the number of edges removed.')
    parser.add_argument('--snapshots', type=int, default=16, help='the number of snapshots.')
    args = parser.parse_args()

    np.random.seed(args.seed)
    graph_name = args.dataset
    dataset = NodePropPredDataset(name=graph_name)
    ...
    np.savez('./dataset/'+ graph_name + '-exp.npz', indices=indices, indptr=indptr, degree=degree, Us = Us, Vs = Vs,
